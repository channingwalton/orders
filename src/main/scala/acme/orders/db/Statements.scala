...
object Statements:
  ...
  def selectOrder(orderId: OrderId): Query0[Order] =
    sql"""
  ...
  def selectOrdersByUser(userId: UserId): Query0[Order] =
    sql"""
      SELECT id, user_id, product_id, status, created_at, updated_at
      FROM orders
  ...
  def updateOrderStatus(orderId: OrderId, status: OrderStatus, updatedAt: Instant): Update0 =
    sql"""
      UPDATE orders
  ...
  def insertSubscription(subscription: Subscription): Update0 =
    sql"""
  ...
  def selectSubscriptionsByUser(userId: UserId): Query0[Subscription] =
    sql"""
