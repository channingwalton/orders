...
class OrderServiceImpl[F[_]: MonadThrow, G[_]](
  ...
  def getOrder(orderId: OrderId): F[Option[Order]] =
    store.commit(store.findOrder(orderId))

  def getUserOrders(userId: UserId): F[List[Order]] =
    store.commit(store.findOrdersByUser(userId))

  def getUserSubscriptions(userId: UserId): F[List[Subscription]] =
    store.commit(store.findSubscriptionsByUser(userId))
  ...
  private def validateProduct(productId: ProductId): F[Unit] =
    Product.values.find(_.id == productId) match
      case Some(_) => ().pure[F]
      case None => ServiceError.InvalidProduct(productId).raiseError[F, Unit]

  private def createSubscriptionForOrder(order: Order, now: Instant): F[Subscription] =
    Product.values.find(_.id == order.productId) match
      case Some(product) =>
        ...
        subscription.pure[F]
      case None => 
        ServiceError.InvalidProduct(order.productId).raiseError[F, Subscription]
